...
var (
	...
	// Additional metrics for block and transaction sizes.
	// Sizes are tracked in bytes to preserve detail; you can convert to MB
	// in Prometheus by dividing by 1024^2.
	blockSizeBytesGauge = metrics.GetOrRegisterGauge("chain/block/size_bytes", nil)
	txCountGauge        = metrics.GetOrRegisterGauge("chain/tx/count", nil)
	txSizeBytesGauge    = metrics.GetOrRegisterGauge("chain/tx/size_bytes", nil)
)
		...
		return lachesis.BlockCallbacks{
			...
			EndBlock: func() (newValidators *pos.Validators) {
				...
				blockFn := func() {
					...
					// Compute block- and tx-level size metrics. We approximate block
					// size as the sum of its transaction sizes, which is sufficient
					// for monitoring growth and pressure.
					var txSizeBytes int64
					for _, tx := range evmBlock.Transactions {
						txSizeBytes += int64(tx.Size())
					}
					blockSizeBytesGauge.Update(txSizeBytes)
					txCountGauge.Update(int64(len(evmBlock.Transactions)))
					txSizeBytesGauge.Update(txSizeBytes)

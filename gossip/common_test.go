...
func newTestEnv(firstEpoch idx.Epoch, validatorsNum idx.Validator) *testEnv {
	...
	for i := idx.Validator(0); i < validatorsNum; i++ {
		...
		key := makefakegenesis.FakeKey(vid)
		_ = valKeystore.Add(pubkey, key.Bytes(), validatorpk.FakePassword)
	...
	go func() {
		for b := range newBlocks {
			...
			for i, tx := range b.Block.Transactions {
				if sig := tx.RawPQCSignatureValues(); len(sig) != 0 {
					mu.Lock()
...
func (env *testEnv) Transfer(from, to idx.ValidatorID, amount *big.Int) *types.Transaction {
	...
	tx, err := types.SignTxPQC(tx, env.EthAPI.signer, key)
	if err != nil {
		panic(err)
	}
...
func (env *testEnv) privateKey(n idx.ValidatorID) pqc.PQCPrivateKey {

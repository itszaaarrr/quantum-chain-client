...
func transaction(nonce uint64, gaslimit uint64, key pqc.PQCPrivateKey) *types.Transaction {
	return pricedTransaction(nonce, gaslimit, big.NewInt(1), key)
}

func pricedTransaction(nonce uint64, gaslimit uint64, gasprice *big.Int, key pqc.PQCPrivateKey) *types.Transaction {
	tx, _ := types.SignTxPQC(types.NewTransaction(nonce, common.Address{}, big.NewInt(100), gaslimit, gasprice, nil), types.PQCSignerv1{}, key)
	return tx
}

func pricedDataTransaction(nonce uint64, gaslimit uint64, gasprice *big.Int, key pqc.PQCPrivateKey, bytes uint64) *types.Transaction {
	data := make([]byte, bytes)
	rand.Read(data)

	tx, _ := types.SignTxPQC(types.NewTransaction(nonce, common.Address{}, big.NewInt(0), gaslimit, gasprice, data), types.PQCSignerv1{}, key)
	return tx
}

func dynamicFeeTx(nonce uint64, gaslimit uint64, gasFee *big.Int, tip *big.Int, key pqc.PQCPrivateKey) *types.Transaction {
	tx, _ := types.SignNewPQCTx(key, types.LatestSignerForChainID(params.ArmchainConfig.ChainID), &types.PQCTxV1{
		ChainID:    params.ArmchainConfig.ChainID,
		Nonce:      nonce,
		GasTipCap:  tip,
		GasFeeCap:  gasFee,
		Gas:        gaslimit,
...
func setupTxPool() (*TxPool, pqc.PQCPrivateKey) {
	return setupTxPoolWithConfig(params.ArmchainConfig)
}

func setupTxPoolWithConfig(config *params.ChainConfig) (*TxPool, pqc.PQCPrivateKey) {
	statedb, _ := state.New(common.Hash{}, state.NewDatabase(rawdb.NewMemoryDatabase()), nil)
	blockchain := &testBlockChain{statedb, 10000000, new(event.Feed)}

	key, _ := crypto.GenerateKeyPQC()
...
func deriveSender(tx *types.Transaction) (common.Address, error) {
	return types.Sender(types.PQCSignerv1{}, tx)
}
...
func TestStateChangeDuringTransactionPoolReset(t *testing.T) {
	...
	var (
		key, _     = crypto.GenerateKeyPQC()
		address    = crypto.PQCPubkeyToAddress(key.PublicKey())
		statedb, _ = state.New(common.Hash{}, state.NewDatabase(rawdb.NewMemoryDatabase()), nil)
		trigger    = false
	)
	...
	pool := NewTxPool(testTxPoolConfig, params.ArmchainConfig, blockchain)
	defer pool.Stop()
...
func TestTransactionNegativeValue(t *testing.T) {
	...
	tx, _ := types.SignTxPQC(types.NewTransaction(0, common.Address{}, big.NewInt(-1), 100, big.NewInt(1), nil), types.PQCSignerv1{}, key)
	from, _ := deriveSender(tx)
	testAddBalance(pool, from, big.NewInt(1))
...
func TestTransactionChainFork(t *testing.T) {
	...
	addr := crypto.PQCPubkeyToAddress(key.PublicKey())
	resetState := func() {
		statedb, _ := state.New(common.Hash{}, state.NewDatabase(rawdb.NewMemoryDatabase()), nil)
...
func TestTransactionDoubleNonce(t *testing.T) {
	...
	signer := types.PQCSignerv1{}
	tx1, _ := types.SignTxPQC(types.NewTransaction(0, common.Address{}, big.NewInt(100), 100000, big.NewInt(1), nil), signer, key)
	tx2, _ := types.SignTxPQC(types.NewTransaction(0, common.Address{}, big.NewInt(100), 1000000, big.NewInt(2), nil), signer, key)
	tx3, _ := types.SignTxPQC(types.NewTransaction(0, common.Address{}, big.NewInt(100), 1000000, big.NewInt(1), nil), signer, key)
...
func TestTransactionDropping(t *testing.T) {
	...
	account := crypto.PQCPubkeyToAddress(key.PublicKey())
...
func TestTransactionPostponing(t *testing.T) {
	...
	keys := make([]pqc.PQCPrivateKey, 2)
	accs := make([]common.Address, len(keys))

	for i := 0; i < len(keys); i++ {
		keys[i], _ = crypto.GenerateKeyPQC()
		accs[i] = crypto.PQCPubkeyToAddress(keys[i].PublicKey())

		testAddBalance(pool, crypto.PQCPubkeyToAddress(keys[i].PublicKey()), big.NewInt(50100))
	}
...
func testTransactionQueueGlobalLimiting(t *testing.T, nolocals bool) {
	...
	pool := NewTxPool(config, params.ArmchainConfig, blockchain)
	defer pool.Stop()

	// Create a number of test accounts and fund them (last one will be the local)
	keys := make([]pqc.PQCPrivateKey, 5)
	for i := 0; i < len(keys); i++ {
		keys[i], _ = crypto.GenerateKeyPQC()
		testAddBalance(pool, crypto.PQCPubkeyToAddress(keys[i].PublicKey()), big.NewInt(1000000))
	}
	...
	if nolocals {
		...
		if queued := pool.queue[crypto.PQCPubkeyToAddress(local.PublicKey())].Len(); uint64(queued) != 3*config.GlobalQueue {
...
func testTransactionQueueTimeLimiting(t *testing.T, nolocals bool) {
	...
	local, _ := crypto.GenerateKeyPQC()
	remote, _ := crypto.GenerateKeyPQC()

	testAddBalance(pool, crypto.PQCPubkeyToAddress(local.PublicKey()), big.NewInt(1000000000))
	testAddBalance(pool, crypto.PQCPubkeyToAddress(remote.PublicKey()), big.NewInt(1000000000))
	...
	statedb.SetNonce(crypto.PQCPubkeyToAddress(remote.PublicKey()), 2)
	statedb.SetNonce(crypto.PQCPubkeyToAddress(local.PublicKey()), 2)
	<-pool.requestReset(nil, nil)
...
func TestTransactionAllowedTxSize(t *testing.T) {
	...
	//   - recipient == 20 bytes
	//   - value     <= 32 bytes
	//   - signature == 3732 bytes (pqc-mldsa)
	// All those fields are summed up to at most 3880 bytes.
	baseSize := uint64(3880)
...
func TestTransactionPoolRepricingKeepsLocals(t *testing.T) {
	...
	// Create a number of test accounts and fund them
	keys := make([]pqc.PQCPrivateKey, 3)
	for i := 0; i < len(keys); i++ {
		keys[i], _ = crypto.GenerateKeyPQC()
		testAddBalance(pool, crypto.PQCPubkeyToAddress(keys[i].PublicKey()), big.NewInt(1000*1000000))
	}
...
func TestDualHeapEviction(t *testing.T) {
	...
	add := func(urgent bool) {
		...
		for i := range txs {
			...
			key, _ := crypto.GenerateKeyPQC()
			testAddBalance(pool, crypto.PQCPubkeyToAddress(key.PublicKey()), big.NewInt(1000000000000))
			if urgent {
...
func TestTransactionDeduplication(t *testing.T) {
	...
	key, _ := crypto.GenerateKeyPQC()
	testAddBalance(pool, crypto.PQCPubkeyToAddress(key.PublicKey()), big.NewInt(1000000000))
...
func testTransactionJournaling(t *testing.T, nolocals bool) {
	...
	pool.Stop()
	statedb.SetNonce(crypto.PQCPubkeyToAddress(local.PublicKey()), 1)
	blockchain = &testBlockChain{statedb, 1000000, new(event.Feed)}
...
func BenchmarkInsertRemoteWithAllLocals(b *testing.B) {
	...
	remoteKey, _ := crypto.GenerateKeyPQC()
	remoteAddr := crypto.PQCPubkeyToAddress(remoteKey.PublicKey())

...
// ImportRawKey stores the given hex encoded PQC key into the key directory,
// encrypting it with the passphrase.
func (s *PrivateAccountAPI) ImportRawKey(privkey string, password string) (common.Address, error) {
	key, err := crypto.HexToPQC(privkey)
	if err != nil {
		return common.Address{}, err
	}
	ks, err := fetchKeystore(s.am)
	...
	acc, err := ks.ImportPQC(key, password)
...
func (s *PrivateAccountAPI) signTransaction(ctx context.Context, args *TransactionArgs, passwd string) (*types.Transaction, error) {
	...
	//Note: PQC signature scheme being used
...
// Sign calculates an PQC signature for:
// keccak256("\x19Ethereum Signed Message:\n" + len(message) + message).
//
// Note, the produced signature conforms to the mldsa44 signature scheme (as of now)
//
...
func (s *PrivateAccountAPI) Sign(ctx context.Context, data hexutil.Bytes, addr common.Address, passwd string) (hexutil.Bytes, error) {
	...
	// PQC signature scheme being used
	signature, err := wallet.SignTextWithPassphrase(account, passwd, data)
	if err != nil {
		log.Warn("Failed data sign attempt", "address", addr, "err", err)
		return nil, err
	}
	// deprectad: signature[crypto.RecoveryIDOffset] += 27 // Transform V from 0/1 to 27/28 according to the yellow paper
	// pqc sig of size 3732 returned
...
//
// # Note, the signature must conform to the mldsa signature scheme
//
// https://github.com/ethereum/go-ethereum/wiki/Management-APIs#personal_ecRecover (modified for armchain api)
func (s *PrivateAccountAPI) EcRecover(ctx context.Context, data, sig hexutil.Bytes) (common.Address, error) {
	//validation of sig (for now only mldsa supported)
	sigStruct, err := mldsa.BytesToSig(sig)
	if err != nil {
		return common.Address{}, err
	}

	if !crypto.ValidatePQCSignatureValue(*sigStruct) {
		return common.Address{}, fmt.Errorf("invalid pqc signature")
	}
	// Recover the public key and the Ethereum address
	pubkey, _, err := crypto.FromPQCSig(sigStruct.Bytes())
	if err != nil {
		return common.Address{}, err
	}
	signer := crypto.PQCPubkeyToAddress(pubkey)
...
// TODO_PQC: refactor logic to support armchain wallet using mldsa
...
type RPCTransaction struct {
	...
	R                *hexutil.Big      `json:"-"`
	S                *hexutil.Big      `json:"-"`
	Signature        hexutil.Bytes     `json:"signature"` // PQC signature is a slice of bytes with (sig + pub key)
}
...
// Sign calculates an PQC(mldsa) signature for:
// keccack256("\x19Ethereum Signed Message:\n" + len(message) + message).
//
// # Note, the produced signature conforms to the mldsa signature scheme
//
// The account associated with addr must be unlocked.
//

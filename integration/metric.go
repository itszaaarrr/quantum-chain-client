...
func (db *DBProducerWithMetrics) OpenDB(name string) (kvdb.Store, error) {
	...
	// Metric names in Prometheus must match the regex [a-zA-Z_:][a-zA-Z0-9_:]*.
	// Our DB names may contain characters (like '-') that are invalid in a
	// Prometheus metric name.  Normalise the DB name that is embedded into the
	// metric *name* so that the exported metrics remain parseable.
	//
	// Example:
	//   name        = "epoch-156"
	//   metricName  = "epoch_156"
	//   full metric = "opera_chaindata_epoch_156_disk_read"
	...
	dm.diskSizeGauge = metrics.GetOrRegisterGauge("opera/chaindata/"+metricName+"/disk/size", nil)

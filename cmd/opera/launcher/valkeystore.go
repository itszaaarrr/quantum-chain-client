...
func addFakeValidatorKey(ctx *cli.Context, key pqc.PQCPrivateKey, pubkey validatorpk.PubKey, valKeystore valkeystore.RawKeystoreI) {
	// add fake validator key
	if key != nil && !valKeystore.Has(pubkey) {
		err := valKeystore.Add(pubkey, key.Bytes(), validatorpk.FakePassword)
		if err != nil {
